...
#Reda
jeps = ["VIPABH", "HuResupport"]
@ABH.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await ABH(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await ABH(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await ABH(functions.channels.EditTitleRequest(
                channel=await ABH.get_entity(event.chat_id),
        ...
        try:
            await ABH(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await ABH.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await ABH.upload_file(profile_pic)
        try:
            await ABH(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
...
#Reda
@ABH.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in ABH.iter_profile_photos(event.chat_id, limit=1) :
                    await ABH(
